# Toolkit (Pro) — READY-MADE tools. USE THESE; do NOT write your own analysis code.
...
2. Holes (least-squares circle fit + classification hole vs boss/outline vs slot;
   center/r/fit-err/z-range per cluster):
   `bash <TK>/bl.sh <TK>/holes.py <parts_dir> <out.json> [part ...]`
3. Screw/standoff profiler — long axis, which end is the head, head/shaft radii, thread length:
   `bash <TK>/bl.sh <TK>/screw_profile.py <parts_dir> [part ...]`
4. Part views — ortho top/front/side + 2 persp PNGs per part:
   `bash <TK>/bl.sh <TK>/render_parts.py <parts_dir> <out_dir> [part ...]`
   CAD line drawings with labeled mm grid (read coordinates straight off the image):
...
5. Manual: `pdftoppm -png -r 300 manual.pdf <prefix>`, zoom/enhance:
   `python3 <TK>/crop.py <in.png> <out.png> --box x0 y0 x1 y1 --scale 2 [--invert --autocontrast]`
6. ASSEMBLE — builds everything from a layout JSON; renders hero/top/front/low/center
...
Workflow: inventory + holes + profiler + edge views -> read manual -> decide placements ->
layout JSON -> assemble -> view renders + alignment report -> fix numbers -> repeat -> finish.
